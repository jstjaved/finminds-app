                    ...
                    {rows.map((r) => (
                      <tr key={r.company_id} className="border-b border-line last:border-0">
                        ...
                        <td className="py-3 text-right">
                          <Link href={`/market/${r.company_id}`} className="text-xs font-semibold text-teal hover:underline">Trade</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                {(transactions || []).map((t) => {
                  ...
                  return (
                    <div key={t.id} className="flex justify-between text-sm text-slate py-2 border-b border-line last:border-0">
                      <span>{co.logo} {isSell ? "Sold" : "Bought"} {t.qty} {co.name}</span>
                      <span className={`font-mono ${isSell ? "text-tealDeep" : "text-ink"}`}>{isSell ? "+" : "-"}{formatCoins(t.qty * t.price)}</span>
                    </div>
                  );
                })}
              </div>
