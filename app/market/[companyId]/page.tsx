...
import { getSimulatedPrice, getDailyChangePct, getPriceHistory } from "@/lib/priceSimulation";
...
function StatCard({ label, value, tooltip }: { label: string; value: string | number; tooltip: string }) {
  return (
    <div className="card py-3">
      <div className="text-[10px] text-slate flex items-center gap-1">{label} <InfoTooltip text={tooltip} /></div>
...
export default async function CompanyPage({ params }: { params: { companyId: string } }) {
  ...
  const [{ data: c }, { data: profile }, { data: holding }] = await Promise.all([
    supabase.from("companies").select("*").eq("id", params.companyId).single(),
    supabase.from("profiles").select("wallet").eq("id", user!.id).single(),
    supabase.from("holdings").select("qty").eq("profile_id", user!.id).eq("company_id", params.companyId).maybeSingle(),
  ]);
  if (!c) return <div className="p-6">Company not found.</div>;

  const price = getSimulatedPrice(c);
  const chg = getDailyChangePct(c);
  const up = chg >= 0;
  const history = getPriceHistory(c, 14);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="card">
              ...
              <div className="mt-4">
                <div className="text-[11px] text-slate mb-1 flex items-center gap-1">
                  14-day trend <InfoTooltip text="Simulated for practice — this app doesn't use real market data." />
                </div>
                <PriceTrend history={history} up={up} />
              </div>
            ...
            <div>
              <div className="text-xs text-slate mb-2">Key stats (simulated, for learning — not live financial data)</div>
          ...
          <div className="lg:sticky lg:top-20 h-fit">
            <div className="card">
              <div className="font-display font-bold text-ink mb-1">Trade</div>
              <TradeForm companyId={c.id} price={price} wallet={profile!.wallet} ownedQty={holding?.qty ?? 0} />
            </div>
          </div>
        </div>
      </div>
